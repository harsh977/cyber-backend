...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python test_forgot_password.py <email>")
        sys.exit(1)
    ...
    try:
        ...
        # Call FastAPI POST endpoint for forgot password
        print("Attempting to initiate password reset...")

        response = client.post("/auth/forgot-password", json={"email": email})

        # Check the response status and print it
        if response.status_code == 200:
            print(f"Password reset link sent successfully to '{email}':", response.json())
        else:
