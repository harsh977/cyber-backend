...
REDIS_DECODING = os.getenv("REDIS_DECODING", "false").lower() == "true"
...
redis_client = redis.StrictRedis(
    ...
    decode_responses = REDIS_DECODING                            
)
...
async def login(user: User, remember_me: Optional[bool] = False):
...
async def verify_otp(request: VerifyOTPRequest):
    try:
        ...
        db = await mongo_connection.connect()
        # Changed this line to properly check collection existence
        if USER_COLLECTION not in await db.list_collection_names():
...
async def reset_password(data: ResetPasswordRequest):
    try:
        # Manually pass the token to the verify_token function
    ...
    await collection.update_one({"email": email}, {"$set": {"password": hashed_pw}})
    
    return {"message": "Password updated successfully"}



@router.post("/auth/toggle-tfa")
async def toggle_tfa(email: EmailStr, tfa_status: bool):
    db = await mongo_connection.connect()
    collection = db[USER_COLLECTION]
    ...
    # Update the tfa_enabled status based on the status passed by frontend
    await collection.update_one({"email": email}, {"$set": {"tfa_enabled": tfa_status}})
    
    if tfa_status:
        return {"message": "Two-factor authentication has been enabled for the user."}
    else:
        return {"message": "Two-factor authentication has been disabled for the user."}
